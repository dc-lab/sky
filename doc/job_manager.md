...
ttl = время жизни задачи (таска), по истечению которого оно будет принудительно завершено.
command = команда запуска задачи 
input_files = идентификаторы файлового ресурса и пути куда его нужно положить
```
...
curl https://sky.io/api/jobs -X POST --data '{
    "tasks": [
        ...
        {"command": "ps ax > processes", output_files=["processes"]}
    ]
}'
```
